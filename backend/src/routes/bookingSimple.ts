...
router.post('/create', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    logActivity({ req, action: 'CREATE_BOOKING', entityType: 'BOOKING', entityId: booking.id, details: { customerName: data.customerName, customerPhone: normalizedPhone } });
...
router.post('/quick-sale', requireAuth, requireStaffOrAdmin, async (req, res) => {
  try {
    ...
    logActivity({ req, action: 'CREATE_QUICK_SALE', entityType: 'BOOKING', entityId: booking.id });
    return res.status(201).json({
      success: true,
      booking,
    });
