...
export default function POSDashboard() {
  ...
  const canUseManagerConsole = isAdmin || isStaff;
  ...
  useEffect(() => {
    if (!showManagerConsole) return;
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') setShowManagerConsole(false);
    };
    ...
    <div className="mc-root lg:h-screen lg:overflow-hidden">
      <AdminHeader
        ...
        navItems={[
          {
            label: 'Manager',
            onClick: () => setShowManagerConsole(true),
      ...
      <main
        className="mc-one-screen mx-auto px-4 sm:px-8 2xl:pl-[224px] 2xl:pr-[244px] 2xl:px-0 py-4 lg:py-3 max-w-[1800px] 2xl:max-w-none lg:h-[calc(100dvh-56px)] lg:overflow-hidden"
      >
      ...
      {showManagerConsole && canUseManagerConsole && (
        <div
          className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm p-3 sm:p-5"
          role="dialog"
          aria-modal="true"
          aria-label="Manager Console"
        >
          <div className="mc-panel h-full max-w-[1180px] ml-auto p-0 flex flex-col shadow-2xl">
            <div className="mc-dialog-header">
              <div className="min-w-0">
                <div className="mc-section-label">Manager Console</div>
                <div className="mc-meta mt-1">Customers, bookings, and manager-only controls</div>
              </div>
              <button
                type="button"
                className="mc-chip ml-auto"
                onClick={() => setShowManagerConsole(false)}
                aria-label="Close Manager Console"
              >
                <X className="h-3.5 w-3.5" />
                Close
              </button>
            </div>
            <div className="flex-1 min-h-0 overflow-y-auto mc-scroll-thin p-4">
              <ManagerPanel />
            </div>
          </div>
        </div>
